...
struct opus_mem_a2a_cco_window_view {
    char* winBase;
    unsigned int stride4G;
    int lsaRank;
};

__device__ inline void* opus_mem_a2a_lsa_peer_ptr(void* win_handle, int peer_lsa_rank) {
    auto* w = reinterpret_cast<const opus_mem_a2a_cco_window_view*>(win_handle);
    return w->winBase + ((static_cast<unsigned long long>(peer_lsa_rank) * w->stride4G) << 32);
}

__device__ inline int opus_mem_a2a_lsa_rank(void* win_handle) {
    return reinterpret_cast<const opus_mem_a2a_cco_window_view*>(win_handle)->lsaRank;
}

// GEMM -> direct A2A reduce variant for opus_gemm_mem_gemm_a2a.
// It reduces the fp32 split-K workspace exactly like splitk_reduce_kernel, then
// stores C columns directly into the destination peer's LSA window:
//   peer_window[src_rank, M, shard_n], peer = col / shard_n.
template<int VEC_ = 16, int BLOCK_ = 64, typename D_OUT = __bf16>
__global__ void splitk_reduce_a2a_kernel(
    const opus_splitk_ws_handle* __restrict__ ws_handle,
    void*        __restrict__ a2a_win,
    int split_k, int M, int N, int batch,
    int padded_M, int padded_N,
    int shard_n, int rank_count)
{
#ifdef __HIP_DEVICE_COMPILE__
#if defined(__gfx950__)
    const float* __restrict__ workspace =
    ...
    static_assert(STEP * sizeof(D_OUT) == 16,
                  "D_OUT must divide a 128-bit store boundary cleanly");
    static_assert(VEC % STEP == 0,
                  "VEC must be a multiple of STEP");
    ...
    using opus::slice;
    using opus::number;
    const int peer = n_base / shard_n;
    const int local_col = n_base - peer * shard_n;
    const int src_rank = opus_mem_a2a_lsa_rank(a2a_win);

    if (n_base + VEC <= N && peer < rank_count && local_col + VEC <= shard_n) {
        D_OUT* peer_base = reinterpret_cast<D_OUT*>(opus_mem_a2a_lsa_peer_ptr(a2a_win, peer));
        auto g_a2a = opus::make_gmem(
            peer_base,
            (unsigned int)((size_t)batch * rank_count * M * shard_n * sizeof(D_OUT)));
        const int dst_idx = ((b * rank_count + src_rank) * M + m) * shard_n + local_col;
        opus::static_for<VEC / STEP>([&](auto g_c_idx) {
            constexpr int g = decltype(g_c_idx)::value;
            g_a2a.template store<STEP>(
                slice(out, number<g * STEP>{}, number<(g + 1) * STEP>{}),
                dst_idx + g * STEP);
        });
    } else if (n_base < N) {
        #pragma unroll
        for (int t = 0; t < VEC; ++t) {
            const int col = n_base + t;
            const int elem_peer = col / shard_n;
            const int elem_col = col - elem_peer * shard_n;
            if (col < N && elem_peer < rank_count) {
                D_OUT* peer_base = reinterpret_cast<D_OUT*>(opus_mem_a2a_lsa_peer_ptr(a2a_win, elem_peer));
                auto g_a2a = opus::make_gmem(
                    peer_base,
                    (unsigned int)((size_t)batch * rank_count * M * shard_n * sizeof(D_OUT)));
                const int dst_idx = ((b * rank_count + src_rank) * M + m) * shard_n + elem_col;
                g_a2a.template store<1>(out[t], dst_idx);
            }
        }
    }
#else
