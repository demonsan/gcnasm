...
static constexpr int kSdmaIssuerBlock = 0;

__device__ inline unsigned flat_load_u32(const unsigned* ptr) {
    return *reinterpret_cast<const volatile unsigned*>(ptr);
}

__device__ inline unsigned long long cycle_clock() {
    return static_cast<unsigned long long>(__builtin_readcyclecounter());
}

__device__ inline unsigned long long* block0_cycle_slots(opus_a2a_gemm_kargs kargs) {
    auto* ready = static_cast<unsigned*>(kargs.ready_local);
    return reinterpret_cast<unsigned long long*>(ready + kargs.rank_count);
}
...
__device__ inline void signal_rotate_ready(opus_a2a_gemm_kargs kargs, int dst_rank, int part, int tid) {
    if (tid == 0) {
...
__device__ inline void sdma_put_owned_shard_to_all_peers(opus_a2a_gemm_kargs kargs, int bx) {
    if (bx != kSdmaIssuerBlock) {
        return;
    }
    const size_t shard_bytes = static_cast<size_t>(kargs.m) * kargs.k_shard * sizeof(D_A);
    const size_t shard_offset = static_cast<size_t>(kargs.my_rank) * shard_bytes;
    mori::cco::ccoSdma sdma{kargs.dev_comm};
    auto* recv_win = reinterpret_cast<mori::cco::ccoWindow_t>(kargs.recv_a_win);
    for (int step = 0; step < kargs.rank_count - 1; ++step) {
        const int dst_rank = (kargs.my_rank - 1 - step + kargs.rank_count) & 7;
        sdma.put<mori::cco::ccoCoopBlock>(
            dst_rank,
            recv_win,
            shard_offset,
            recv_win,
...
__device__ inline void sdma_quiet_owned_shard_to_all_peers(opus_a2a_gemm_kargs kargs, int bx) {
    if (bx != kSdmaIssuerBlock) {
        return;
    }
    mori::cco::ccoSdma sdma{kargs.dev_comm};
    for (int step = 0; step < kargs.rank_count - 1; ++step) {
        const int dst_rank = (kargs.my_rank - 1 - step + kargs.rank_count) & 7;
        sdma.quiet<mori::cco::ccoCoopBlock>(dst_rank);
    }
}
...
void a2a_gemm_lsa_direct_read_kernel(opus_a2a_gemm_kargs kargs) {
    ...
    if (bx == kSdmaIssuerBlock && tid == 0) {
        block0_cycle_slots(kargs)[0] = cycle_clock();
    }

    auto sdma_quiet_owned_shard_and_signal = [&](int shard_iter) {
        if (bx != kSdmaIssuerBlock) {
            return;
        }
        const int dst_rank = (kargs.my_rank - 1 - shard_iter + kargs.rank_count) & 7;
        mori::cco::ccoSdma sdma{kargs.dev_comm};
        sdma.quiet<mori::cco::ccoCoopBlock>(dst_rank);
        signal_rotate_ready(kargs, dst_rank, kargs.my_rank, tid);
    };
...
#define LOAD_B_TILE(slot, half) ([&]() { auto mem = B_TILE(slot, half); return load<T::VEC_B>(mem, u_rb); }())
    ...
    if (kargs.sdma_fused) {
        sdma_put_owned_shard_to_all_peers<D_A>(kargs, bx);
    }
    ...
    if (kargs.sdma_fused && shard_iter + 1 < shard_outer_loops) {
        s_waitcnt_vmcnt(0_I);
        if (wave_id_m == 0) __builtin_amdgcn_s_barrier();
        sdma_quiet_owned_shard_and_signal(shard_iter);
    }
    }
    ...
    if (bx == kSdmaIssuerBlock && tid == 0) {
        block0_cycle_slots(kargs)[1] = cycle_clock();
    }
}

__global__ __launch_bounds__(512, 1)
void a2a_sdma_allgather_kernel(opus_a2a_gemm_kargs kargs) {
    using namespace a2a_gemm_direct_read;
    const int bx = opus::block_id_x();
    sdma_put_owned_shard_to_all_peers<bf16_t>(kargs, bx);
    sdma_quiet_owned_shard_to_all_peers(kargs, bx);
}
